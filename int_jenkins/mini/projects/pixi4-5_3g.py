...
class project(AllProject):
	...
	def initConfFromXls(self, conf):
		...
		ProjectConfig = commands.getoutput('/local/int_jenkins/bin/MT6580_X_GetVerInfo '+ versionStr[0:3]+'X -All').split('\n')
		argvList = []
	...
	def moveDBtoTemp(self, conf):
		...
		if(version[2] >= 'U' and version.__contains__('_')):
			versionstr=version[0:4]
                BAND = conf.getConf('BAND', 'which BAND version to deliver? <CN|EU1|EU2|US1|US2|US3|US4|US5|US6|2M|AWS|2G|LATAM2G>')
		if( BAND[0:2] == 'EU'):
			BAND='EU'
		else:
			BAND='US'
