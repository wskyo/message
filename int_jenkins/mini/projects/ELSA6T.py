...
class project(AllProject):
	...
	def initConfFromXls(self, conf):
		...
		if(versionStr[-2] >= 'U' and not versionStr.__contains__('-')):
			BAND = conf.getConf('BAND', 'which BAND version to deliver? <CN|EU1|EU2|US1|US2|2M|AWS|2G|LATAM3G|LATAM2G|US0-1S|US0-2S|US1-1S|US1-2S|US2-2S|US3-1S|US3-2S|US4-1S|US4-2S|US5-2S|US5-1S>')
		ProjectConfig = commands.getoutput('/local/int_jenkins/bin/MT6750_X_GetVerInfo '+ versionStr[0:3]+'X -All').split('\n')
		argvList = []
		...
		argvList.extend(['-manifestprefix', 'int/%s/'%ProjectConfig[1].strip()])
	...
	def moveDBtoTemp(self, conf):
		...
		if(version[2] >= 'U' and version.__contains__('_')):
			versionstr=version[0:4]
                BAND = conf.getConf('BAND', 'which BAND version to deliver? <CN|EU1|EU2|US1|US2|US3|US4|US5|2M|AWS|2G|LATAM2G>')
		if( BAND[0:2] == 'EU'):
			BAND='EU'
		else:
			BAND='US'
