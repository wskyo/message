...
def copyPatch(project):
    ...
    docmd('ssh {0} "rm {1}/{2}/*.tar.gz"'.format(project.copyFrom, project.localPath, project.project), exp=exp, exit=False)
...
def sendMail(project, patch, emailLink):
    ...
    tmp = u'<p align=\'Left\'>Patch file already put to share server : \\\\10.92.32.12\RDhzKM\\5-SWD2\B-技术文档\MTK\patch\%s</b></font><br/></p>' % project.project
    html.append(tmp)
    html.append(
    ...
    ccto = ['<shuzhong.cui.hz@tcl.com>', '<yu.he.hz@tcl.com>', '<ying.chen.hz@tcl.com>']
    ...
    msg.attach(contMsg)
    sendtolist = ['ying.chen.hz@tcl.com', 'yu.he.hz@tcl.com']
    s = smtplib.SMTP('mailsz.tct.tcl.com')
    s.login(project.username, project.password)
    s.set_debuglevel(0)
    #s.sendmail(project.email, 'ying.chen.hz@tcl.com', msg.as_string())
