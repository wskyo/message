...
class AllProjectNew(ChangeUtils, MailUtils, InterfaceUtils, SheetUtils, CheckUtils, DBUtils, ReleaseStyle,CheckWifi):
	...
	def run(self):
		...
		pushdir(buildDir)
		version = conf.getConf('version', 'current version')
		if conf.getConf('genchecklist', 'Generate mini test check list <yes|no>', self.isBigVersion(version)) == 'yes':
		...
		if buildok == 'yes':
			if conf.getConf('genreleasenote', 'Generate release note <yes|no>', 'yes') == 'yes':
                                if self.isMiniVersion(version) == 'yes':
                                    ...
                                    self.createReleaseNoteFromBugzilla(conf)
		else:
			## load compiling log
		...
		if buildok == 'yes' and (conf.getConf('isBigVersion', 'version is big version') == 'yes' or conf.getConf('isDailyVersion', 'version is Daily version') == 'yes'):			
                    os.system('/local/int_jenkins/sortresult/projects/ssh.sh %s %s %s %s'%(self.username,self.ip,self.password,conf.getConf('prlistprojname', 'Project name in PR List file name')))			
                    docmd('scp /tmp/ReleaseNote_%s_SW%s.xls user@10.92.35.20:/var/www/data/Release_Note/%s' % ( conf.getConf('prlistprojname', 'Project name in PR List file name'), conf.getConf('version','current version'),conf.getConf('prlistprojname', 'Project name in PR List file name')))
		...
		#insert buildinfo
		if conf.getConf('prlistprojname', 'Project name in PR List file name') == 'pixi3-4':
		...
		if buildok == 'yes' and (conf.getConf('isBigVersion', 'version is big version') == 'yes' or self.isMiniVersion(version) == 'yes'):
			...
			self.InsertWeeklyReport(conf,self.bugzillaloginname,comment)
			release_type = 'release'
			os.system('python /local/int_jenkins/misc/CreateTaskForRealeaseVersionInPRSM.py "%s" "%s" "%s" "%s" "%s"' % (codebranch,version,release_type,comment,self.bugzillaloginname))
		if buildok == 'yes' and conf.getConf('isBigVersion', 'version is big version') == 'yes':
