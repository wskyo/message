...
class InterfaceUtils:
	...
	def initSomeConf(self, conf):
		...
		conf.getConf('isMiniVersion','is mini version <yes|no>', self.isMiniVersion(version))
		conf.getConf('isBigVersion','is big version <yes|no>', self.isBigVersion(version))
		conf.getConf('isDailyVersion','is Daily version <yes|no>', self.isDailyVersion(version))
		self.getBranch(conf)
		self.getManifestPrefix(conf)
		self.mailSubject(conf)
		self.getTelewebVersion(conf)
		self.getBase(conf)
		self.getBuildModemVersion(conf)
		pmDict = readFile('#','/local/int_jenkins/superspam/conf/common.conf')
		##info = UserInfo()
	...
	def preLoad(self, conf):
		...
		if version[0] == '3' and version[1] == 'E':
			nameRule = self.getNameRule_1()
		else:
	...
	def loadcompilelog(self, conf):
		...
		projectName = conf.getConf('project', 'Project name in check list file name')
		      ...
		      docmd( 'zip %s/attach/android_log.zip /local/build/%s-release/android.log ' % (self.tempdir,projectName))
	...
	def getTelewebVersion(self, conf):
		return conf.getConf('telewebversion', 'download teleweb version', 'MTK_SP 3.8.8')
