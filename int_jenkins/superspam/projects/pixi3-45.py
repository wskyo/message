...
class project(AllProjectNewAlm):
	...
	def initConfFromXls(self, conf):
		...
		argvList.extend(['-owner','yan.xiong_xiaoling.luo'])
		argvList.extend(['-mtkproject',self.ProjectConfig[9].strip()])
		if versionStr[2] == '6' or versionStr[2] == 'T': 
			argvList.extend(['-mtkBaseLine','JRDHZ72_WE_72_S1A_KK_ALPS.KK1.MP6'])
		else:
			argvList.extend(['-mtkBaseLine','JRDHZ72_WE_72_S1_KK_ALPS.KK1.MP6.V1'])
	...
	def getNameRule(self):
		return 'Pixi3-45-Software_Naming_Rule_v3.6.xls'
	...
	def getModemVersion(self, version):
		##beetlelite CU
		if(version[1:2] == '6' ):
			return 'JRDHZ72_WE_72_S1A_KK_GPRS_HSPA_MOLY_WR8_W1315_MD_WG_MP_V36'
		else:###other beetlelite project
			return 'JRDHZ72_WE_72_S1_KK_GPRS_HSPA_MOLY_WR8_W1315_MD_WG_MP_V36_F1'
	...
	def getMtkBaseLineCheck(self, version, conf):
		...
		if checkResult[0] == 0 and len(checkResult[1]) >= 1:
			...
			return 'True'
	def getImgeFromOtherServer(self,version):
		if not os.path.isdir('/local/release/pixi3-45-release/'):
			os.system('mkdir -p /local/release/pixi3-45-release/')
		#try:
		docmd_noexit('rsync -rl zhaoshie@10.92.35.31:/local/release/pixi3-45-release/v%s /local/release/pixi3-45-release/' %version)
		docmd_noexit('rsync -rl zhaoshie@10.92.35.31:/local/build/pixi3-45-release/v%s/out/target/product/pixi3_45_android.log /local/build/pixi3-45-release/android.log' %version)
		#except:
		#docmd_noexit('rsync -rl zhaoshie@10.92.35.31:/local/release/pixi3-45-release/v%s /local/release/pixi3-45-release/' %version)
		#docmd_noexit('rsync -rl zhaoshie@10.92.35.31:/local/build/pixi3-45-release/v%s/out/target/product/pixi3_45_android.log /local/build/pixi3-45-release/android.log' %version)		

	def checkCode(self,builddir,version):
		if not os.path.isdir(builddir):
			os.system('mkdir -p %s' % builddir)
		os.chdir(builddir)
		xmlName = builddir + "/.repo/manifests/int/pixi3-45/v"+ version+'.xml'
		print "xmlName",xmlName
		if os.path.exists(builddir + "/.repo") == False: 
			repoinit = pexpect.spawn('repo init -u git@10.92.32.10:sdd1/manifest -m int/pixi3-45/v%s.xml' % version)
			repoinit.sendline()
			...
			os.system('repo sync')
		elif os.path.exists(xmlName) == False:
			os.system('repo init -u git@10.92.32.10:sdd1/manifest -m int/pixi3-45/v%s.xml' % version)
			os.system('repo sync')
		else:
			os.system('repo sync')
			#os.system('repo sync vendor')
			#if version[2] == '6' or version[2] == 'T':
				#os.system('repo sync packages/apps/SimpleHomeScreen')
