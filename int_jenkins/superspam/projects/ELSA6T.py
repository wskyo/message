...
class project(AllProjectNewAlm):
	...
	def initConfFromXls(self, conf):
		...
		if(versionStr[-2] >= 'U' and not versionStr.__contains__('-')):
			self.BAND = conf.getConf('BAND', 'which BAND version to deliver? <CN|EU1|EU2|US1|US2|2M|AWS|2G|LATAM3G|LATAM2G|US0-1S|US0-2S|US1-1S|US1-2S|US2-2S|US3-1S|US3-2S|US4-1S|US4-2S|US5-2S|US5-1S|US_TF-1S|US_TF-2S>')
		...
		argvList.extend(['-manifestprefix', 'int/%s/'%self.ProjectConfig[1].strip()])
		...
		argvList.extend(['-owner','shie.zhao_xiaofen.zhong'])
		argvList.extend(['-mtkproject',self.ProjectConfig[9].strip()])
		argvList.extend(['-mtkBaseLine','alps-mp-n0.mp7-V1'])
	...
	def getNameRule(self):
		return 'ELSA6T-Software_Naming_Rule_v0.6.xls'
	...
	def doMiniTestAddFromProject(self, worksheet, conf, checkListConf):
		...
		self.miniTestCheckItem(worksheet, conf, checkListConf, 'Fingerprint', 'Enrol fingerprint and use fingerprint to unlock screeen,set any application as quick application access and can lanuch it use fingerprint')
	...
	def getModemVersion(self, version):
		...
		if(version[1:2] == '7' or version[1:2] == 'X'):
			return 'MOLY.LR11.W1603.MD.MP.V1'
