...
class CtsMailUtils:
	...
	def getMailHeadHtml(self, conf):
		...
		html += '<title>[CTS Test Result]%s v%s</title>' %(projectName,version)
		html += '</head>'
		return html
	...
	def getBodyHalfHtml(self, conf):
		...
		html = '<p>Dear all,</p>'
                html += '<p align=\'Left\'>'+ '%s v%s CTS run finish!' %(projectName,version)
                html += '<p align=\'Left\'>'+ 'Please check its result ! Thankyou!'
		html += '<br><br>'
		return html
	...
	def sendMail(self, conf, html):
		...
		version = conf.getConf('version', 'project current version')
		projectName = conf.getConf('project', 'Project name in check list file name')
		msg['Subject'] = conf.getConf('mailsubject', 'Mail subject','<title>[CTS Test Result]%s v%s</title>' %(projectName,version))
		...
		for attachFile in glob.glob(self.tempdir+'/*.zip'):
		...
		if smtpAddrSet != None and smtpAddrSet.__len__() == 0:
			smtpAddrSet.add('<yinfang.lai@tcl.com>')
