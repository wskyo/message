...
cp ${builddir}/out/target/product/${project}/md1rom.img ${releasedir}/flashtool
...
if [ ${#version} -gt 4 ] || [[ ${version:2:1} =~ [U-Z] ]] ; then
   if [ -f flashtool/system.img ]; then ln -s flashtool/system.img Y${main}${BN}${sub}BV00.mbn; fi   
