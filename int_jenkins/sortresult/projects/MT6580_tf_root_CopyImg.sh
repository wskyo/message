...
if [ -e ${builddir}/out/target/product/${project}/signed_bin/boot-sign.img ]
then 
	cp ${builddir}/out/target/product/${project}/signed_bin/boot-sign.img ${releasedir}/flashtool/rootboot.img
else 
	echo "there is no boot.img when build"
 	exit 0
fi
if [ -e ${builddir}/out/target/product/${project}/signed_bin/lk-sign.bin ]
then 
	cp ${builddir}/out/target/product/${project}/signed_bin/lk-sign.bin ${releasedir}/flashtool/rootlk.bin
else 
