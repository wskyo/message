...
cp ${builddir}/modem/out_modem/*.dll ${releasedir}/flashtool
zip -r ${builddir}/out_modem.zip ${builddir}/modem/out_modem 
cp ${builddir}/out_modem.zip ${releasedir}/flashtool

cd ${releasedir}
if [ -d flashtool ]; then
   chmod -R 777 flashtool/*
fi
if [ -f flashtool/MT65*_Android_scatter_emmc.txt ]; then ln -s flashtool/MT65*_Android_scatter_emmc.txt K${main:0:3}EMMCAN00.sca; fi
...
case $band in
    ...
    "panasonic" )
        BN='PA';;
    * )
        echo "No such band! (EU|US|US1|US2|2M|AWS|LATAM2G|LATAM3G|panasonic)"
        exit 0;;
esac
