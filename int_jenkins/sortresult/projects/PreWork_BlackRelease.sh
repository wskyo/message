...
function wrong_security {
    ...
    files="vendor/mediatek/proprietary/bootable/bootloader/lk/img_hdr_lk.cfg vendor/mediatek/proprietary/custom/pixi4_4_tf/security/tee/BOOT_SEC_VER.txt vendor/mediatek/proprietary/custom/pixi4_4_tf/security/recovery/BOOT_SEC_VER.txt vendor/mediatek/proprietary/bootable/bootloader/preloader/platform/mt6580/src/security/inc/PL_CFG.h"
    ...
    chat6="PRELOADER_SEC_VERSION"
    cmdout=''
    tmp=0
    search_chats="$chat1 $chat2 $chat3 $chat4 $chat5 $chat6"
    for file in $files
    do
...
function set_root_userdebug {
     ...
     # add "LOCAL_CFLAGS += -DALLOW_ADBD_ROOT=1"
     sed -i 's/ifeq (yes, \$(strip \$(MTK_BUILD_ROOT)))/LOCAL_CFLAGS += -DALLOW_ADBD_ROOT=1\n&/g' system/core/adb/Android.mk
}
...
function disable_DM_verity {
     #  comment out DM-verity configuration
     sed -i "s/PRODUCT_SYSTEM_VERITY_PARTITION :=/# &/g" device/mediatek/mt6580/device.mk
     sed -i "s/\$(call inherit-product, build/# &/g" device/mediatek/mt6580/device.mk
}
...
case $1 in
    ...
    "Offical_root")
        ...
        #BEGIN:modiby by yange.zhang 20160819
        #use userdebug rather than eng, to enable dm-verity
        #root
        set_root_userdebug
        #END: modiby by yange.zhang 20160819
...
#add by chunlei 20160814 begin
    ...
    "Offical_no_secure_boot_sign")
        ...
        ;;
     #add by chunlei 20160814 begin
    "Offical_Partial_Security_Build")
        echo "Offical_Partial_Security_Build: begin ..."
        set_root_userdebug
        wrong_security
        disable_DM_verity
        echo "Offical_Partial_Security_Build: done!"
        ;;
      #end
    *)
        echo "Usage $0 {Offical_root|Offical_rootUnsigned|Offical_Wrong_SecurityVersion|Offical_no_secure_boot_sign|update_version|Offical_debug_SW|Offical_Partial_Security_Build}"
        ;;
esac
