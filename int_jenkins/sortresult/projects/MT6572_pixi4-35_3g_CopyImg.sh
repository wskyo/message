...
cp ${builddir}/out/target/product/${project}/pro_info.bin ${releasedir}/flashtool
...
cp ${builddir}/out/target/product/${project}/obj/CGEN/APDB_MT6572_S01_L1.MP6_${APDBWEEK_NO} ${releasedir}/flashtool
...
#cp ${builddir}/mediatek/custom/common/modem/jrdhz72_we_72_s1_kk/mcddll.dll ${releasedir}/flashtool
cp ${builddir}/modem/mtk_rel/PIXI4_35_HSPA/DEFAULT/tst/database/MCDDLL.dll ${releasedir}/flashtool
cp ${builddir}/modem/mtk_rel/PIXI4_35_HSPA/DEFAULT/tst/database/mcddll.dll ${releasedir}/flashtool
if [[ ${version:2:1} =~ [U-Z] ]]; then
...
if [ ${#version} -gt 4 ] || [[ ${version:2:1} =~ [U-Z] ]] || [[ ${version:2:1} =~ [6] ]]; then
   if [ -f flashtool/system.img ]; then ln -s flashtool/system.img Y${main}${BN}${sub}AZ00.mbn; fi   
   if [ -f flashtool/secro.img ]; then  ln -s flashtool/secro.img X${main}0${perso}${sub}AZ00.mbn; fi
...
if [ -f flashtool/BPLGUInfoCustomAppSrcP* ]; then ln -s flashtool/BPLGUInfo* O${main}0${perso}${sub}AZ00.db; fi
if [ -f flashtool/pro_info.bin ]; then ln -s flashtool/pro_info.bin O${main}0${perso}${sub}BS00.mbn; fi
