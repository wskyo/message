...
APDBWEEK_NO=`cat ${buildblackdir}/device/jrdchz/${project}/ProjectConfig.mk | awk '{print $3}' | grep W16`
cp ${buildblackdir}/out/target/product/${project}/obj/CGEN/APDB_MT6580_S01_L1.MP6_${APDBWEEK_NO} ${releasedir}/flashtool
...
#cp ${buildblackdir}/mediatek/custom/common/modem/jrdhz72_we_72_s1_kk/mcddll.dll ${relaseblackdir}/flashtool
cp ${buildblackdir}/modem/mtk_rel/PIXI4_35_HSPA/DEFAULT/tst/database/MCDDLL.dll ${relaseblackdir}/flashtool
cp ${buildblackdir}/modem/mtk_rel/PIXI4_35_HSPA/DEFAULT/tst/database/mcddll.dll ${relaseblackdir}/flashtool
...
if [ ${#version} -gt 4 ] || [[ ${version:2:1} =~ [S-Z] ]]; then
    if [ -f flashtool/system.img ]; then ln -s flashtool/system.img Y${main}${BN}${sub}AN00.mbn; fi   
...
if [ -f flashtool/APDB_MT6580* ]; then ln -s flashtool/APDB_MT6580* A${main}0${perso}${sub}AZ00.db; fi
