...
if [ -z "$3" ]; then
	echo "Please give the path name"
	exit 1
fi
...
../../out/host/linux-x86/bin/mkuserimg.sh -s $path/out/target/product/$2/system   $path/out/target/product/$2/system.img ext4 system $system_size ../../out/target/product/$2/root/file_contexts
fi
else
