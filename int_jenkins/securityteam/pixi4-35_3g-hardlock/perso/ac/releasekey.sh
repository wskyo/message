...
cd -
processApk() {
    if [ -f ../../out/target/product/$4/system/custpack/$1 ]
    then
	touch ../../out/target/product/$4/system/custpack/$1.signed
	chmod 777 ../../out/target/product/$4/system/custpack/$1.signed
	./signapk.sh $1 $2 $3 $4

	######## add by yanxiang.zhang at 20151230 start ################
	# SignResultInfos=$(jarsigner -verify -verbose -certs ../../out/target/product/$4/system/custpack/$1.signed)
	# for info in $SignResultInfos
	# do
	# #echo "info:$info"
	# SignKey=$(echo $info |grep "O=TCT"  | awk '{print $1}' )
	# len=$(echo ${#SignKey})
	# #echo "len:$len"
	# if [ $len != 0 ];then
	# echo "$1 sign successfully!"
	# break
	# else
	# echo "Error:$1 sign failed!"
	# fi
	# done
	######## add by yanxiang.zhang at 20151230 end ################
	
	touch ../../out/target/product/$4/system/custpack/$1.signed_aligned
	chmod 777 ../../out/target/product/$4/system/custpack/$1.signed_aligned
	../../out/host/linux-x86/bin/zipalign -f 4 ../../out/target/product/$4/system/custpack/$1.signed ../../out/target/product/$4/system/custpack/$1.signed_aligned

	mv ../../out/target/product/$4/system/custpack/$1.signed_aligned ../../out/target/product/$4/system/custpack/$1
	
	rm ../../out/target/product/$4/system/custpack/$1.signed
    else
        echo "out/target/product/$4/system/custpack/$1 not found"
    fi
}
