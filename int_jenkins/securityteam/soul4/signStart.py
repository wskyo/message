...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_jenkins/securityteam/soul4/misc_sign_soul4_custpack.sh .')
                os.chdir('build/target/product')
                docmd('zip -r security_bc.zip security')
                os.chdir(self.tmpdir)
		docmd('rm -rf build/target/product/security/*')
		docmd('cp /local/int_jenkins/securityteam/soul4/TCL_ReleaseKeys.zip build/target/product/security/')
		os.chdir('build/target/product/security')
		docmd('unzip TCL_ReleaseKeys.zip')
	
	def build(self):

		docmd('./makeMtk -t -o=TARGET_BUILD_VARIANT=user soul4 dist')
	...
	def signapk(self):
		...
		child = pexpect.spawn('bash ./misc_sign_soul4_custpack.sh out/dist/soul4-target_*.zip out/dist/sign_target_apks.zip')
		...
		os.chdir(self.tmpdir)
		docmd('mv out/target/product/soul4/custpack.img out/target/product/soul4/custpack.img_unsign')
		docmd('mv out/target/product/soul4/system.img out/target/product/soul4/system.img_unsign')
		docmd('mv out/target/product/soul4/recovery.img out/target/product/soul4/recovery.img_unsign')
		docmd('mv out/target/product/soul4/boot.img out/target/product/soul4/boot.img_unsign')
		docmd('cp out/dist/sign_img/custpack.img out/target/product/soul4/')
		docmd('cp out/dist/sign_img/system.img out/target/product/soul4/')
		docmd('cp out/dist/sign_img/recovery.img out/target/product/soul4/')
		docmd('cp out/dist/sign_img/boot.img out/target/product/soul4/')
		docmd('perl mediatek/build/tools/jrd_magic.pl soul4')
